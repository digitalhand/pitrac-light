...
bool GolfSimConfiguration::ReadValues() {
	...
	// Resolve relative motion detect path against PITRAC_ROOT
	if (!LibCameraInterface::kCameraMotionDetectSettings.empty() &&
		LibCameraInterface::kCameraMotionDetectSettings[0] != '/') {
		std::string root = GetPiTracRootPath();
		if (!root.empty()) {
			LibCameraInterface::kCameraMotionDetectSettings = root + "/" + LibCameraInterface::kCameraMotionDetectSettings;
		}
	}
